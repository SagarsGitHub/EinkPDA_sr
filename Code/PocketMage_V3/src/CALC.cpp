    ...
    switch (CurrentCALCState) {
      ...
      case CALC3:
        ...
        // Space Recieved
          ...
          if (CurrentFrameState != &conversionScreen){
            ...
            CurrentFrameState = &conversionScreen;
            TOUCH().setDynamicScroll(CurrentFrameState->scroll);
            TOUCH().setPrevDynamicScroll(CurrentFrameState->prevScroll);
            newLineAdded = true;
          } 
        }
        // CR Recieved
        ...
        // add non-special characters to line
          ...
          if (KB().getKeyboardState() == SHIFT) KB().setKeyboardState(FUNC);
        }
        ...
        if (currentMillis - OLEDFPSMillis >= 16) {
          ...
          if (TOUCH().getLastTouch() == -1) {
            ...
            if (TOUCH().getDiff()) TOUCH().setPrevDynamicScroll(TOUCH().getDynamicScroll());
            updateScroll(CurrentFrameState,TOUCH().getPrevDynamicScroll(),TOUCH().getDynamicScroll());     
          }
      ...
      case CALC0:
        ...
        // LEFT (scroll up)
          ...
          if (TOUCH().getDynamicScroll() < CurrentFrameState->source->size() - (9 + SCROLL_MAX)){
             //dynamicScroll += SCROLL_MAX;
          } else if (TOUCH().getDynamicScroll() < CurrentFrameState->source->size() - (9 + SCROLL_MED)){
             //dynamicScroll += SCROLL_MED;
          } else if (TOUCH().getDynamicScroll() < CurrentFrameState->source->size() - (9 + SCROLL_SML)){
             //dynamicScroll += SCROLL_SML;
          } else if (TOUCH().getDynamicScroll() < CurrentFrameState->source->size() - 10) {
            //dynamicScroll++;
          }
          newLineAdded = true; 
          updateScroll(CurrentFrameState,TOUCH().getPrevDynamicScroll(),TOUCH().getDynamicScroll());                            
        ...
        // RIGHT (scroll down)
          ...
          if (TOUCH().getDynamicScroll() > (SCROLL_MAX +1)){
           // dynamicScroll -= SCROLL_MAX;
          }else if (TOUCH().getDynamicScroll() > (SCROLL_MED +1)){
            //dynamicScroll -= SCROLL_MED;
          } else if (TOUCH().getDynamicScroll() > (SCROLL_SML +1)){
           // dynamicScroll -= SCROLL_SML;
          } else if (TOUCH().getDynamicScroll() > 1){
           // dynamicScroll--;
          } 
          updateScroll(CurrentFrameState,TOUCH().getPrevDynamicScroll(),TOUCH().getDynamicScroll());     
          newLineAdded = true;
    ...
    if (calcSwitchedStates == 1){
      ...
      if (CurrentCALCState != CALC3 && CurrentCALCState != CALC4)  {   
        ...
        if (CurrentCALCState == CALC1){
          ...
          TOUCH().setDynamicScroll(helpScreen.scroll);
          updateScroll(CurrentFrameState,TOUCH().getPrevDynamicScroll(),TOUCH().getDynamicScroll());  
          TOUCH().setPrevDynamicScroll(-1);
        }
      }
